...
- **`npm run lint` covers all of `/src` — keep the explicit path.** The script is `expo lint .`, not `expo lint`. Without a path argument `expo lint` walks only `src`, `app`, and `components` relative to its working directory, so from `/src` it would lint `app/` and `components/` and silently skip `hooks/`, `utils/`, `api/`, `providers/`, `plugins/`, `testUtils/`, and the top-level `__tests__/` — 46% of the app (DEX-95). Test files run a relaxed rule set; see `docs/testing.md`.
- **A stale `.expo/types/router.d.ts` can fail `npm run typecheck` on routes that exist.** Typed routes are generated by `expo start`/`expo export`, and the file is gitignored, so after adding a route it reports the new `Href` as invalid until the dev server regenerates it. CI has no `.expo/` at all and is unaffected. If `typecheck` flags a route you just added, start the dev server once before believing it.
