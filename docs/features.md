...
- **RNGH can't arbitrate with a native ScrollView's pan** (`blocksExternalGesture`
  only blocks its stand-in), so a pressed card pauses Week's scroller via UI-thread
  `setNativeProps` (DEX-207) — that narrows the race to about a frame, not to zero.
