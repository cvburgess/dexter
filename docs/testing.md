...
- **`react-native-audio-api`'s native stub won't do:** the official `mock` export has no `AudioManager.disableSessionManagement` and a 44100 default sample rate that balloons the Breathe tests' buffers, so `jest.setup.js` carries a hand-rolled inert one (its `decodeAudioData` never settles — component tests don't wait on it). To assert on playback, mock the module per test and resolve the decode deferred there (`hooks/__tests__/useHoroscopeAudio.test.ts`); the deferred lives inside the `jest.mock` factory because the decode fires at module scope, before the test file's own constants exist.
