...
app/
  ...
  (app)/
    ...
    (tabs)/
      ...
      today/               # "Today" tab — sun icon; a `DayViewSwitcher` toggles the day's content between Tasks (the task list, preceded by a tappable habit tracker `components/HabitTracker` → `HabitRing`, gated on `enableHabits`), Notes (`components/NotesView`), Journal (`components/JournalView`), and Calendar (`components/CalendarView`, a scrollable timeline of the day's calendar events, gated on `enableCalendar`)
