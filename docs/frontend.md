...
- **A morning task-list step was built here first and removed.** `DayTaskList`
  ...
  The evening's Open tasks step below is **not** a reversal of it: see there
  for the axis the two differ on.
...
- **Scope is the ritual's day and only what is still open** — `selectOpenTasksForDate`,
  which is `selectTasksForDate` narrowed by the same `isCompletionStatus` the
  backlog scope uses. Stragglers from earlier days stay the morning Backlog
  step's business; pulling them in makes the evening list the thing it exists to
  close.
