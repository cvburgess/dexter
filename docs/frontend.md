...
**Large screens** (`hooks/useIsMultiPane.ts`, width ≥ `utils/breakpoints.ts`'s `TWO_PANE_MIN_WIDTH`) show a multi-column layout instead (DEX-40): Tasks is always visible (capped at `PANE_MAX_WIDTH`), and Notes/Journal/Calendar are toggleable columns to its right — no swiping (`DayNav`'s arrows/picker are the only way to change days). `components/DayPaneToggles.tsx` renders one round `GlassIconButton` per enabled surface in the header (reusing `DayViewSwitcher`'s `VIEW_META` icons/labels), tinted via the button's `active` prop; pressing one calls `hooks/useTodayPanes.ts`'s `togglePane`, which persists which panes are open to the device (`AsyncStorage`, like `useEnabledDeviceCalendars` — not the synced `preferences` row, since it's a per-device layout choice) and defaults every pane open. Notes and Journal share one bordered, tabbed pane (`components/NotesJournalTabs.tsx`) rather than separate columns — a small manila-folder-style tab bar (rounded top corners, the active tab's card color merging into the content below) appears only when both are enabled; with just one, its content fills the pane with no tab bar. `NotesView` takes an `inset` prop (`false` here) so its card runs flush to the column instead of double-inset with the small-screen gutter. Calendar gets its own narrower cap (`CALENDAR_PANE_MAX_WIDTH`) and a bordered card, and is pinned to the row's right edge via `marginLeft: "auto"` so it stays rightmost even when Notes/Journal isn't rendered. A "+" `GlassIconButton` beside the pane toggles opens the create-task modal scheduled for the viewed day (mirroring `NewTaskButton`, but pushing directly since the date is already in scope here rather than read back from the module store).
