...
app/
  ...
  (app)/
    ...
    (tabs)/
      ...
      settings/            # "Settings" tab — gear icon; grouped list of subview rows (Account, Appearance, Tasks, Calendars, Habits, Journal, Notes, Licenses). Log out lives in the Account subview; Licenses lists the app's open-source dependencies (generated by `npm run licenses` into utils/licenses.json); the rest are placeholders
