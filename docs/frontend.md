...
app/
  ...
  (app)/
    ...
    (tabs)/
      ...
      settings/            # "Settings" tab — gear icon; grouped list of subview rows (Account, Appearance, Tasks, Calendars, Habits, Journal, Notes, Licenses). Log out lives in the Account subview; Appearance is the theme picker; Habits toggles habit tracking and inline-edits habits (`components/HabitRow`); Licenses lists the app's open-source dependencies (generated by `npm run licenses` into utils/licenses.json); Tasks, Calendars, Journal, and Notes are still placeholders
