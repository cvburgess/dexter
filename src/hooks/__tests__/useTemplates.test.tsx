...
describe("useTemplates", () => {
  ...
  describe("saveTaskAsTemplate", () => {
    it("copies the task's fields with no schedule and no dates", async () => {
      const view = await renderUseTemplates();

      act(() => {
        view.result.current[1].saveTaskAsTemplate(task);
      });
      ...
      expect(mockCreateTemplate).toHaveBeenCalledWith(expect.anything(), {
        ...
        title: "Trip packing",
        schedule: null,
        // The blueprint records what the steps are, not how far this one task
        // got — so the done subtask arrives without its status.
        subtasks: [{ id: "sub-1", title: "Passport" }],
      });
    });

    // Linking would make MoreMenu read the task as repeating and would let the
    // mcp-server's `delete_task` take the template down with the task.
    it("leaves the task it was taken from untouched", async () => {
      const view = await renderUseTemplates();
    ...
    it("hands the created template back so the caller can open its editor", async () => {
      const view = await renderUseTemplates();
      const onSuccess = jest.fn<void, [TTemplate]>();

      act(() => {
        view.result.current[1].saveTaskAsTemplate(task, { onSuccess });
      });

      await waitFor(() => expect(onSuccess).toHaveBeenCalled());
      expect(onSuccess.mock.calls[0][0]).toEqual({ id: "template-1" });
    });
  });
});
