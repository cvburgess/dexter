...
describe("density tiers", () => {
  ...
  it("uses the compact tier on a wide web viewport", () => {
    ...
    expect(result.current.fonts.body.fontSize).toBe(13);
...
describe("palette invariants", () => {
  ...
  it.each(names)("%s draws its chrome on distinct surfaces", (name) => {
    const { background, card, surfaceSunken, border } = themes[name].colors;

    // Opaque, so a border stays visible over any surface and the rail reads as
    // a different plane rather than a tint of whatever is behind it.
    for (const color of [surfaceSunken, border]) {
      expect(color).toMatch(/^#[0-9a-f]{6}$/);
    }
    expect(surfaceSunken).not.toBe(background);
    expect(surfaceSunken).not.toBe(card);
  ...
  it("blends the accent over the theme background at 80%", () => {
    // #fcb700 at 80% over dark's #191e24 background.
    expect(themes.dark.colors.priorityMuted[0]).toBe("#cf9807");
    // #ff627d at 80% over dexter's #f7f1e7 background.
    expect(themes.dexter.colors.priorityMuted[1]).toBe("#fd7f92");
  });
});
